...
## ~={orange}1. Concorrenza, Parallelismo e Sistemi Distribuiti=~
...
### ~={yellow}Concorrenza=~ (Livello Logico e di Astrazione)
...
### ~={yellow}Parallelismo=~ (Livello Fisico e di Prestazioni)
...
### ~={yellow}Programmazione Distribuita =~(Livello di Rete)
...
### ~={yellow}Panoramica delle Architetture Fisiche=~
...
|**Architetture Ibride**|Combinazione di core differenti per bilanciare prestazioni consumi energetici.|• **P-core**: Grandi e veloci per task single-thread pesanti.<br><br>  <br><br>• **E-core**: Piccoli ed efficienti per task in background.|Processori desktop e mobile recenti (es. Intel Alder Lake e successivi)|
|**Processori Eterogenei & Coprocessori**|Chip che integrano componenti dedicati a compiti specifici oltre alla CPU.|• **GPU / GPGPU**: Calcolo matriciale massivamente parallelo.<br><br>  <br><br>• **SoC**: CPU, GPU, NPU, ISP e SEP in un unico chip.<br><br>  <br><br>• **Hardware IA Nativo**: Wafer integrato per eliminare i colli di bottiglia nei trasferimenti dati.|• GPU/GPGPU<br><br>  <br><br>• Apple Silicon (M-series)<br><br>  <br><br>• Cerebras WSE-3 (900k core, 44 GB SRAM on-chip)|
...
## ~={orange}3. Classificazione delle Architetture: La Tassonomia di Flynn=~
...
1. **SISD (Single Instruction, Single Data):** 
	Il classico modello Von Neumann monocore. Un'istruzione alla volta elabora un dato alla volta.
...
### ~={yellow}Sottocategorie del Modello MIMD=~
...
## ~={orange}4. Classificazione Concettuale dei Sistemi Concorrenti=~
...
### ~={yellow}Sistema + Ambiente: Trasformativi vs Reattivi=~
...
### ~={yellow}Struttura Interna del Sistema=~
...
## ~={orange}5. Mappa concettuale della Programmazione Concorrente=~
